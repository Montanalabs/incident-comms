...
Untrusted input is a status update; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a status update
- **Closed type:** `type Decision = PublishStatus(StatusKind) | HoldStatus` where `StatusKind` is closed
- **Sink / capability:** `grant publishStatus irreversible` → `commit { publishStatus(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/incident-comms/demo.sh
```
...
- `incident-comms_safe.wave` — the correct design.
- `incident-comms_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
